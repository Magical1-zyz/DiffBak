...
def render_layer(
    ...
    # Multi-material path: select material per pixel using interpolated face material index
    if getattr(mesh, 'materials', None) is not None and getattr(mesh, 'face_material_idx', None) is not None:
        # Build per-face scalar attribute with the material index and interpolate to pixels
        face_mat_vals = mesh.face_material_idx.float().unsqueeze(-1)  # [F,1]
        gb_matid, _ = interpolate(face_mat_vals[None, ...], rast_out_s, face_normal_indices.int())  # [1,H,W,1]
        # Accumulate buffers over materials using masks
        buffers_accum = None
        for mi, mm in enumerate(mesh.materials):
            # Build a per-pixel mask for material mi
            mi_val = torch.tensor(float(mi), dtype=gb_matid.dtype, device=gb_matid.device)
            mask = (torch.round(gb_matid[..., 0:1]) == mi_val).float()
            # If mask is all zeros, skip
            if not torch.any(mask > 0).item():
                continue
            buffers_i = shade(gb_pos, gb_geometric_normal, gb_normal, gb_tangent, gb_texc, gb_texc_deriv,
                              view_pos, lgt, mm, bsdf)
            # Initialize accumulators on first contribution
            if buffers_accum is None:
                buffers_accum = {k: torch.zeros_like(v) for k, v in buffers_i.items()}
            # Accumulate masked contribution
            for key in buffers_i.keys():
                buffers_accum[key] = buffers_accum[key] + buffers_i[key] * mask
        # If everything skipped (e.g., missing materials), fall back to first material
        if buffers_accum is None:
            buffers_accum = shade(gb_pos, gb_geometric_normal, gb_normal, gb_tangent, gb_texc, gb_texc_deriv,
                                  view_pos, lgt, mesh.materials[0], bsdf)
...
def render_meshes(
    ...
    if background is not None:
        if spp > 1:
            background = util.scale_img_nhwc(background, full_res, mag='nearest', min='nearest')
        accum = torch.cat((background, torch.zeros_like(background[..., 0:1])), dim=-1)
    else:
        accum = torch.zeros(1, full_res[0], full_res[1], 4, dtype=torch.float32, device='cuda')
